...
export async function generateStaticParams() {
  const slugs = await getAllSlugsByRegion("ca");
  return slugs.map((slug) => ({ slug }));
}

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { slug } = await params;
  const post = await getPostBySlugAndRegion(slug, "ca");
  ...
  return {
    ...
    openGraph: {
      title: metaTitle,
      description: metaDescription,
      url: `https://tutorexel.com/ca/blog/${post.slug}`,
      siteName: "TutorExel",
      locale: "en",
      type: "article",
    ...
    alternates: getRegionalAlternates('/blog/' + post.slug, 'ca'),
  };
}

export default async function CaBlogPostPage({ params }: Props) {
  const { slug } = await params;
  const post = await getPostBySlugAndRegion(slug, "ca");
  ...
  const relatedPosts = await getRelatedPosts(slug, post.region || "ca");
  ...
  const blogBreadcrumbSchema = createBreadcrumbSchema([
    { name: "Home", url: "https://tutorexel.com/ca" },
    { name: "Learning Hub", url: "https://tutorexel.com/ca/blog" },
    { name: post.title, url: `https://tutorexel.com/ca/blog/${post.slug}` },
  ]);
